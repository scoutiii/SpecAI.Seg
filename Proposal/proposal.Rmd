...
Not only does this help include spatial information, it also helps reduce processing resources, as each segment can be summarized typically using the average of all pixels in the segment.
...
Additional, this package has several segmentation algorithms in addition to Watershed, such as Slic, Quickshift, and Felzenszwalb.
...
This would also be a novel contribution to the R community because though there are package to load in HSI into R, there are no package for doing HSI segmentation in R.
The existing R packages for working with HSI are \texttt{raster}, \texttt{rhdf5}, and \texttt{rgdal} [@raster, @rhdf5, @rgdal].
However these package don't offer segmentation functionality, and other packages such as \texttt{itcSegment} are too specialized for general HSI segmentation [@itcSegment].
Furthermore, because many of the popular HSI datasets are used in Python, they are store as simple \texttt{.mat} files, thus using proper spatial statistics packages are unnecessary for our proposed package.
...
There are a few potential challenges will need to be handled.
First, data loading.
...
This gradient calculation involves calculating the pairwise distance matrix for a neighborhood of pixels around each pixels, meaning there are a lot of calculations.
In \texttt{SpecAI.Seg} uses multiprocessing to speed up this calculation.
...
Third, if needs be, how to reticulate and Python inside of R.
